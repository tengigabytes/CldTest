...
| **換行** | 不允許 | （目前未實作 — 訊息對話輸入框是單列 inline，留待 Canned/記事本全螢幕版補上） |
| **字數計數位置** | 框內右側 | 編輯區右下角 |
| **OK** | 有候選 → 確認候選 / 無候選 → 確認並回 App | 有候選 → 確認候選 / 無候選 → 送出 |
| **OK 長按 ≥500ms** | 同短按 | 同短按（送出由「OK 在無候選且 buffer 非空」自然觸發） |
...
| **OK** | 有候選 → 確認候選 / 無候選 → 確認並回 App | 有候選 → 確認候選 / 無候選 → 送出（不離開,送完留原 App 等下一則） |
| **OK 長按 ≥500ms** | 同短按 | 同短按 |
...
按下 OK
  │
  └── 有候選字（buffer 非空）→ 確認當前選中候選字
  │
  └── 無候選字（buffer 空）   → action:enter
                                ├── 模式 A：commit + 回 App
                                └── 模式 B：送出當前累積文字（送完留原 App）
```

**為何取消長/短按差異化**:

原規格 v0 設計「模式 B 短按換行 / 長按≥500ms 送出」,但實作後發現:

1. **訊息對話輸入區是單列 inline**,沒 word-wrap;短按插入 `\n` 在 Unifont 下被當作空格 glyph 渲染,視覺異常
2. **WASM ImeLogic OK 鍵 release edge** 在 input 已空時會 emit stray space commit;雙路徑(短=換行/長=送出)會放大此 firmware 行為差異,連續按 OK 會出現多餘空格
3. **使用者心智模型**:在 5-way 裝置上 OK 期望就是「確認當前狀態」— 有候選就選字、沒候選就送出,符合 Meshtastic phone app 操作直覺

實作上:`mie-hal.processKeyTap` 對 OK 鍵跳過 release edge 的 `mie_key`
呼叫,僅在 JS 端依 `_inputBeforePress` 是否空白決定是否派發
`action:enter`。長按 OK 的 hold 事件會被 keyboard-hal 抑制 short tap,
故長按行為 = 短按行為,完全等價。

**換行需求**:Canned Message / 記事本 / Admin Channel 註解 等真正需
要多行的文字框,規劃用「全螢幕模式 B 編輯器」(可 word-wrap、可手動
換行)— 該 widget 上線時再決定換行用哪個鍵(候選:`SYM` 長按 / 雙擊
DOWN / 專屬「↵」鍵未來新增)。

**無視覺提示**:回饋來自送出後文字框清空(訊息對話中會看到自己的訊
息出現在對話流)。
...
最後更新：2026-04-29
版本：v1.1（移除 OK 長/短按差異化:單一 OK 路徑「有候選確認 / 無候選送出」,模式 B 換行待全螢幕編輯器補上）
